...
Boole CreateDirectoryPath(const String& DirectoryPath)
{
    ...
    StringVector FolderVec(Split(DirectoryPath,"/\\"));
    size_t StartIndex = 0;
    String PathAttempt;
...
void RemoveDirectory(const String& DirectoryPath)
{
    if( !rmdir(DirectoryPath.c_str()) ) {
        ...
        throw std::runtime_error("Unknown error removing directory.");
    }
}
...
StringVector GetDirContents(const String& Dir)
{
    ...
    if( ( Directory = opendir( Dir.c_str() ) ) ) {
        ...
        throw std::runtime_error(String("Error listing directory contents"));
    }
}
...
String CombinePathAndFileName(const String& FilePath, const String& FileName)
{
    ...
    String Separator(1,GetDirectorySeparator());
    if( !EndsWith(FilePath,Separator,true) ) {
...
#endif
    { throw std::runtime_error(String("Could not change to directory \"")+ChangeTo+"\" error: "+ConvertToString(errno)); }
}
...
String ResolveDataPathFromString(const String& PathVar)
{
    ...
    {
        throw std::invalid_argument("Attempting to retrieve unknown path variable: \"" + PathVar + "\".");
    }
}
...
String GetLocalAppDataDir()
{
    ...
    if(pw) {
        ...
        throw std::runtime_error("Could not get user information to retrieve app data directory.");
    }
...
String GetShareableAppDataDir()
{
    ...
    if(pw) {
        ...
        throw std::runtime_error("Could not get user information to retrieve home directory.");
    }
#endif
    return "";
}
...
String GetCurrentUserDataDir()
{
    ...
    if(pw) {
        ...
        throw std::runtime_error("Could not get user information to retrieve user data directory.");
    }
#endif
    return "";
}
...
String GetCommonUserDataDir()
{
    ...
    if(pw) {
        ...
        throw std::runtime_error("Could not get user information to retrieve common data directory.");
    }
#endif
    return "";
}
