...
namespace
{
    ...
    void Base64DecodeImpl(const String& EncodedString, BinaryBuffer& Results)
    {
        if(Results.Binary==0 || Results.Size==0)
            { throw std::domain_error("Cannot encode an empty buffer."); }
        ...
        while(Progress<EncodedString.end())
        {
            ...
            if(!IsBase64(*Progress) || !IsBase64(*(Progress+1)) || !IsBase64(*(Progress+2)) || !IsBase64(*(Progress+3)))
                { throw std::domain_error("Base64 contains an invalid character and cannot be decoded."); }
...
BinaryBuffer& BinaryBuffer::operator= (const BinaryBuffer& RH)
{
    if (RH.Binary == this->Binary)
        { throw std::logic_error("Attempted a self assignment of a BinaryBuffer"); }
...
void BinaryBuffer::CreateBuffer()
{
    if(Size)
    {
        ...
        throw std::domain_error("Cannot create a 0 length Buffer.");
    }
}
...
String BinaryBuffer::ToBase64String()
{
    if(!Size || !Binary)
    {
        throw std::domain_error("Cannot encode 0 length or missing buffer.");
    }
...
String BinaryBuffer::ToString()
{
    if(!Size || !Binary)
    {
        throw std::domain_error("Cannot construct string from 0 length or missing buffer.");
    }
...
Whole PredictBinarySizeFromBase64String(String const& EncodedString)
{
    if(EncodedString.size()<4)
        { throw std::invalid_argument("It is not possible to have a base64 string less than 4 bytes in length, but one was received."); }
